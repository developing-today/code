...
var port = flag.Int("port", 8001, "Port to bind app to")
...
func RunStreamServer(ctx context.Context, cmd *cobra.Command, args []string) {
	...
	node.OnConnect(func(client *centrifuge.Client) {
		...
		client.OnPublish(func(e centrifuge.PublishEvent, cb centrifuge.PublishCallback) {
			...
			//if err != nil {
			//log.Printf("[user %s] sent non-json, data: %s", client.UserID(), string(e.Data))
	...
	log.Print("Starting server, visit http://localhost:8001")
	go func() {
...
func SetupNodeHandlers(node *centrifuge.Node) {
	...
	node.OnConnect(func(client *centrifuge.Client) {
		...
		client.OnPublish(func(e centrifuge.PublishEvent, cb centrifuge.PublishCallback) {
			...
			if !client.IsSubscribed(e.Channel) {
        log.Info("client isn't subscribed")
				cb(centrifuge.PublishReply{}, centrifuge.ErrorPermissionDenied)
				return
			}
			...
			err := json.Unmarshal(e.Data, &msg)
			if err != nil {
      log.Info("message failed to unmarshal")
				//cb(centrifuge.PublishReply{}, centrifuge.ErrorBadRequest)
				//return
        msg = clientMessage{
          Input: string(e.Data),
