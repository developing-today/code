...
def run_migration_parallel(conn, **options):
    ...
    print(f"START | Running update to create {length}^3={length*length*length} vertices. |")

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(run_migration_segment, conn, part, total_parts, length, time_now) for part in range(1, total_parts + 1)]
        for future in futures:
            future.result()

    print(f"DONE  | Bulk vertex creation {length}^3={length*length*length} completed. |")
...
def run_migration_segment(conn, part, total_parts, length, start_time):
    ...
    print(f"{total_elapsed} | {part}/{total_parts} | STARTED  | x {x_start}-{x_end} | y {y_start}-{y_end} | z {z_start}-{z_end} | {elapsed} | {elapsed} |")
    ...
    elapsed = now - time_now
    print(f"{total_elapsed} | {part}/{total_parts} | PREPARED | x {x_start}-{x_end} | y {y_start}-{y_end} | z {z_start}-{z_end} | {len(vertex_creations)} | {elapsed} | {elapsed - previous_elapsed} |")
    ...
    previous_elapsed = elapsed
    elapsed = now - time_now
    print(f"{total_elapsed} | {part}/{total_parts} | COMPLETE | x {x_start}-{x_end} | y {y_start}-{y_end} | z {z_start}-{z_end} | {len(vertex_creations)} | {elapsed} | {elapsed - previous_elapsed} |")
