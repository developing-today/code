...
def print_call_info(conn):
    ...
    settings = execute_into_single(conn, "CALL CURRENT_SETTING('var_length_extend_max_depth') RETURN *;")
    print("Var Length Extend Max Depth:", settings)

    settings = execute_into_single(conn, "CALL CURRENT_SETTING('enable_semi_mask') RETURN *;")
...
def sort_migrations(migrations_directory, separators=None):
    migrations = [f.name for f in os.scandir(migrations_directory) if f.is_file() and f.name.endswith('.py')]
...
def is_valid_migration_name(filename, migration_count, counter, valid_new_counters):
    ...
    if version_number <= counter:
        return version_number, f"Version number ({version_number}) lower than or equal to current counter ({counter}): {filename}"

    if version_number > counter + migration_count:
        return version_number, f"Version number ({version_number}) higher than current counter ({counter}) + migration count ({counter + migration_count}) = ({counter + migration_count}): {filename}"

    if any(version_number <= new_counter for new_counter in valid_new_counters):
        return version_number, f"Bad version number: duplicate or lower than another new counter ({version_number}): {filename}"
...
def run_migrations(conn, migrations_directory, migrations, counter):
    ...
    for migration in migrations[counter:]:
        new_counter, error_message = is_valid_migration_name(migration, migration_count, counter, valid_new_counters)
    ...
    if errors:
        error_messages = '\n'.join([f"Migration {name}({counter}): {message}" for name, message, counter in errors])
...
def get_latest_version(conn):
  result = execute_into_single(conn, 'MATCH (v:Version) RETURN v.name, v.counter, v.completions ORDER BY v.counter DESC LIMIT 1;')
  if result is None:
