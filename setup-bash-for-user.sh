...
restore_shell_options() {
  ...
  echo "trap done: restoring shell options"
}
...
if [ "${#args[@]}" -eq 0 ]; then
  users=("$USER")
  printf "%s %s\n" "no user provided: default to current user:" "${users[*]}"
else
  users=("${args[@]}")
fi
...
if command -v parallel >/dev/null 2>&1; then
  printf "%s\n" "processing users method: parallel"
  parallel process_user {} $is_force "$bash_profile_global_content" "$bashrc_global_content" ::: "${users[@]}"
  exit_code=$?
else
  printf "%s\n" "processing users method: sequential"
  for username in "${users[@]}"; do
    process_user "$username" "$is_force" "$bash_profile_global_content" "$bashrc_global_content"
    user_exit_code=$?
