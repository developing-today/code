...
class Notification(gobject.GObject):
    ...
    def _message_filter(self, connection, dbus_message):
        ...
        if (
            dbus_message.get_member() == "Notify"
            and dbus_message.get_interface() == "org.freedesktop.Notifications"
        ):
            try:
                message = Message(dbus_message, Timestamp.now())
            except:
                logger.exception(
                    "Failed to parse dbus message: "
                    + repr(dbus_message.get_args_list())
                )
            else:
                # Discard unwanted messages
                if message.is_volume_notification():
                    return
                if self._blacklist and self._blacklist.get_bool(
                    message.app_name, False
                ):
                    return
