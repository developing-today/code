...
describe("groupCursorsByPosition", () => {
  const makeCursor = (clientID: string, head: number, lastUpdate: number): CursorForMerge => ({
    clientID,
    head,
    anchor: head,
    name: clientID,
  ...
  describe("grouping by position", () => {
    ...
    it("groups cursors at same position", () => {
      const cursors = [makeCursor("alice", 10, 1000), makeCursor("bob", 10, 2000)];
      const groups = groupCursorsByPosition(cursors);
    ...
    it("creates separate groups for different positions", () => {
      const cursors = [makeCursor("alice", 10, 1000), makeCursor("bob", 20, 2000)];
      const groups = groupCursorsByPosition(cursors);
  ...
  describe("sorting groups by position", () => {
    it("sorts groups by position ascending", () => {
      const cursors = [makeCursor("alice", 50, 1000), makeCursor("bob", 10, 2000), makeCursor("carol", 30, 3000)];
      const groups = groupCursorsByPosition(cursors);
...
describe("doGroupsOverlap", () => {
  ...
  describe("overlap detection with mock coords", () => {
    ...
    it("returns true when groups actually overlap", () => {
      // A at pos 0 (0px), tooltip width = 6.3px (1 char * 6.3)
      // B at pos 0 (0px) - same position = definite overlap
      // With MIN_GAP = 2, overlap threshold is 6.3 + 2 = 8.3px
      // posDiff = 0, 0 < 8.3 is TRUE = overlap
    ...
    it("returns false when groups are close but not overlapping", () => {
      ...
      // B at pos 1 (10px)
      // With MIN_GAP = 2, overlap threshold is 6.3 + 2 = 8.3px
      // posDiff = 10, 10 < 8.3 is FALSE = no overlap
  ...
  describe("multiple groups", () => {
    ...
    it("checks all consecutive pairs", () => {
      ...
      // 30 < 40 = TRUE = overlap
      const groups = [makeGroup(0, ["Alexander"]), makeGroup(3, ["Benjamin"])];
      expect(doGroupsOverlap(groups, mockGetLeftCoord)).toBe(true);
    });

    it("returns false if no consecutive pairs overlap", () => {
      const groups = [makeGroup(0, ["A"]), makeGroup(50, ["B"]), makeGroup(100, ["C"])];
      expect(doGroupsOverlap(groups, mockGetLeftCoord)).toBe(false);
    });
  });
...
describe("clusterOverlappingGroups", () => {
  ...
  describe("clustering with mock coords", () => {
    ...
    it("merges groups that actually overlap", () => {
      // "Alexander" at pos 0 (0px), tooltip width = 56.7px (9 chars * 6.3)
      // "B" at pos 5 (50px)
      // With MIN_GAP = 2, overlap threshold is 56.7 + 2 = 58.7px
      // posDiff = 50, 50 < 58.7 is TRUE = overlap, so merge
    ...
    it("merges actually overlapping groups into one cluster", () => {
      ...
      // "Alexander" at pos 0, width ~56.7px (9*6.3)
      // "B" at pos 3 (30px) - within Alexander's overlap threshold (56.7+2=58.7px)
      // posDiff = 30, 30 < 58.7 = TRUE = overlap
    ...
    it("clusters only actually overlapping groups", () => {
      ...
      // C at 100 (no overlap)
      const groups = [makeGroup(0, ["Alexander"]), makeGroup(3, ["B"]), makeGroup(100, ["C"])];
      const result = clusterOverlappingGroups(groups, mockGetLeftCoord);
      expect(result).toHaveLength(2);
