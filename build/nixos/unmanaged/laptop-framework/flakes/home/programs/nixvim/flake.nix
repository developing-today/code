{
  inputs = {
    ...
    nixvim = {
      ...
      inputs.nixpkgs.follows =
        "nixpkgs"; # https://github.com/nix-community/nixvim/issues/1702
    };
  ...
  outputs =
    { nixpkgs, nixvim, flake-utils, neovim-nightly-overlay, ... }@inputs:
    let
      enablePkgs = { ... }@args:
        builtins.mapAttrs (n: v: v // { enable = true; }) args;
      enablePlugins = attrSet:
        if attrSet ? plugins then
          attrSet // { plugins = enablePkgs attrSet.plugins; }
        else
          attrSet;
      enableLspServers = attrSet:
        if attrSet ? lsp && attrSet.lsp ? servers then
          attrSet // {
            lsp = attrSet.lsp // { servers = enablePkgs attrSet.lsp.servers; };
          }
        else
          attrSet;
      enableColorschemes = attrSet:
        if attrSet ? colorschemes then
          ...
          attrSet;
      enableModules = attrSet:
        enableColorschemes (enableLspServers (enablePlugins attrSet));
    in flake-utils.lib.eachDefaultSystem (system:
      ...
      in {
        ...
        homeManagerModules =
          homeManagerModules; # unsure how to overlay nightly here.
